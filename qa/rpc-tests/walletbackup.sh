...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} OFC"
...
function StartThree {
  $OFCCoinD $OFCCoinDARGS $B1ARGS &
  B1PID=$!
  $OFCCoinD $OFCCoinDARGS $B2ARGS &
  B2PID=$!
  $OFCCoinD $OFCCoinDARGS $B3ARGS &
  B3PID=$!
}
