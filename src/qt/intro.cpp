...
void Intro::pickDataDirectory()
{
    ...
    dataDir = settings.value("strOFCDataDir", dataDir).toString();
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/OFCCoin"));
        ...
        while(true)
        {
            ...
            try {
                ...
                QMessageBox::critical(0, tr("OFCCoin"),
                    tr("Error: Specified data directory \"%1\" can not be created.").arg(dataDir));
                /* fall through, back to choosing screen */
            }
        }

        settings.setValue("strOFCDataDir", dataDir);
    }
    /* Only override -datadir if different from the default, to make it possible to
     * override -datadir in the OFCCoin.conf file in the default data directory
     * (to be consistent with OFCCoind behavior)
     */
    if(dataDir != getDefaultDataDirectory())
